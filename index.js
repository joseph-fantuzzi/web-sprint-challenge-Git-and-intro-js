...
function addArtist(array) {
  const myObject = {
    id: 20,
    name: 'Joseph Fantuzzi',
    years: '2000 - 2022',
    genre: 'Web Design',
    nationality: 'Italian',
    bio: 'Going to BloomTech to study web and blockchain development in hopes to land a job in the crypto industry as a lead developer for a company.'
  }
  array.push(myObject);
  return array;
}
...
function lotsOfArt(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    if (array[i].paintings > 100) {
      namesOfArtists.push(array[i].name);
    }
  }
...
function artistInfo(array, name){
  const artistBioArray = [];
  for (let i = 0; i < array.length; i++) {
    if (array[i].name === name) {
      artistBioArray.push(array[i].bio);
    }
  }
  return artistBioArray;
}
...
function artistByCountry(array, nationality){
  ...
  for (let i = 0; i < array.length; i++) {
    if (array[i].nationality === nationality) {
      similarArtists.push(array[i].name);
    }
  }
